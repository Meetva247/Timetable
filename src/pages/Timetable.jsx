...
import {
    ...
    getClassrooms, getFaculty, getDepartments, getAllUniqueSubjects
} from '../data'
...
export default function Timetable() {
    ...
    const allUniqueSubs = getAllUniqueSubjects();
    const getSubObj = (name) => allUniqueSubs.find(s => s.name === name || s.shortCode === name) || { name, shortCode: name };
    ...
    const openSubjectEditor = () => {
        ...
        setSubForm(s.core.map(c => ({
            ...
            hasLab: typeof c === 'object' ? c.hasLab : false,
            labOnly: typeof c === 'object' ? c.labOnly : false
        })))
        setSubModal(true)
    }
    ...
    return (
        <div style={{ maxWidth: 1300, margin: '0 auto', padding: '2rem 1.5rem 4rem' }} className="page-wrapper">
                                                        ...
                                                        <td key={day} rowSpan={rowSpan} style={{ padding: '.4rem .3rem', borderRight: '1px solid var(--border)', verticalAlign: 'top' }}
                                                            ...
                                                            <div onClick={() => openEdit(day, period)}
                                                                ...
                                                                {slot.batches ? (
                                                                    ...
                                                                    <>
                                                                        <div style={{ fontWeight: 800, borderBottom: `1px solid var(--border)`, paddingBottom: '.2rem', marginBottom: '.1rem', display: 'flex', flexDirection: 'column', gap: '.1rem' }}>
                                                                            <span>{getSubObj(slot.subject).shortCode} (LAB)</span>
                                                                            <span style={{ fontSize: '.65rem', fontWeight: 600, opacity: 0.9 }}>{getSubObj(slot.subject).name !== getSubObj(slot.subject).shortCode ? getSubObj(slot.subject).name : ''}</span>
                                                                        </div>
                                                                    ...
                                                                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1, gap: '.25rem' }}>
                                                                        <div style={{ fontWeight: 800 }}>{getSubObj(slot.subject).shortCode || <span style={{ color: 'var(--text-3)', fontStyle: 'italic' }}>— empty —</span>}</div>
                                                                        {slot.subject && <div style={{ fontSize: '.65rem', fontWeight: 600, opacity: 0.9 }}>{getSubObj(slot.subject).name !== getSubObj(slot.subject).shortCode ? getSubObj(slot.subject).name : ''}</div>}
            ...
            <Modal open={subModal} title={`Edit Core Subjects — Semester ${sem}`} onClose={() => setSubModal(false)}>
                ...
                <div key="header" style={{ display: 'grid', gridTemplateColumns: '90px 100px 1fr 50px 40px 40px auto', gap: '.4rem', marginBottom: '.5rem', alignItems: 'center', fontSize: '.75rem', fontWeight: 700, color: 'var(--text-3)', textTransform: 'uppercase' }}>
                    <div>Admin Code</div>
                    <div>Short Code</div>
                    <div>Full Name</div>
                    <div>Cr</div>
                    <div style={{ textAlign: 'center' }}>Lab</div>
                    <div style={{ textAlign: 'center' }}>Only</div>
                    <div></div>
                </div>
                {subForm.map((s, i) => (
                    <div key={s.id} style={{ display: 'grid', gridTemplateColumns: '90px 100px 1fr 50px 40px 40px auto', gap: '.4rem', marginBottom: '.5rem', alignItems: 'center' }}>
                        ...
                        <div style={{ textAlign: 'center' }}>
                            <input type="checkbox" checked={s.hasLab} onChange={e => setSubForm(sf => sf.map((x, j) => j === i ? { ...x, hasLab: e.target.checked } : x))} />
                        </div>
                        <div style={{ textAlign: 'center' }}>
                            <input type="checkbox" checked={s.labOnly} onChange={e => setSubForm(sf => sf.map((x, j) => j === i ? { ...x, labOnly: e.target.checked } : x))} />
                        </div>
                ...
                <button style={{ display: 'inline-flex', alignItems: 'center', gap: '.4rem', padding: '.4rem .85rem', borderRadius: 10, fontSize: '.85rem', fontWeight: 600, cursor: 'pointer', border: '2px solid var(--primary)', background: 'transparent', color: 'var(--primary)', fontFamily: 'inherit', marginTop: '.5rem' }}
                    onClick={() => setSubForm(sf => [...sf, { id: uid(), name: '', shortCode: '', code: '', credits: 0, hasLab: false, labOnly: false }])}>➕ Add Subject</button>
                <div style={{ display: 'flex', gap: '.75rem', justifyContent: 'flex-end', marginTop: '1.5rem' }}>
