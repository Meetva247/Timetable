...
export default function Subjects() {
    ...
    const [editSubjectModal, setEditSubjectModal] = useState({ open: false, subject: null, sem: null, name: '', shortCode: '', code: '', credits: 0, department: '', hasLab: false, labOnly: false })
    ...
    const openEditSubject = (s, sem) => {
        setEditSubjectModal({ open: true, subject: s, sem, name: s.name || '', shortCode: s.shortCode || s.name || '', code: s.code || '', credits: s.credits || 0, department: s.department || getDepartments()[0] || '', hasLab: s.hasLab || false, labOnly: s.labOnly || false })
    }

    const handleUpdateSubject = e => {
        e.preventDefault()
        const { subject, sem, name, shortCode, code, credits, department, hasLab, labOnly } = editSubjectModal
        if (!subject || !sem) return

        const id = subject.code || subject.name
        const success = updateSubjectDetails(sem, id, { name, shortCode, code, credits: Number(credits), department, hasLab, labOnly })

        if (success) {
            showToast(`Updated details for ${name} in Semester ${sem}`, 'success')
            setEditSubjectModal({ open: false, subject: null, sem: null, name: '', shortCode: '', code: '', credits: 0, department: '', hasLab: false, labOnly: false })
            load() // Refresh list
        }
    }
                ...
                <form onSubmit={handleUpdateSubject}>
                    ...
                    <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem' }}>
                        <label style={{ display: 'flex', alignItems: 'center', gap: '.5rem', fontSize: '.9rem', cursor: 'pointer' }}>
                            <input type="checkbox" checked={editSubjectModal.hasLab} onChange={e => setEditSubjectModal(m => ({ ...m, hasLab: e.target.checked }))} />
                            Has Lab
                        </label>
                        <label style={{ display: 'flex', alignItems: 'center', gap: '.5rem', fontSize: '.9rem', cursor: 'pointer' }}>
                            <input type="checkbox" checked={editSubjectModal.labOnly} onChange={e => setEditSubjectModal(m => ({ ...m, labOnly: e.target.checked }))} />
                            Lab Only
                        </label>
                    </div>
                    <p style={{ fontSize: '.75rem', color: 'var(--warning)', marginBottom: '1.5rem' }}>⚠️ This update will <strong>only affect Semester {editSubjectModal.sem}</strong>. Changes will not apply to other semesters.</p>
                    <div style={{ display: 'flex', gap: '.75rem', justifyContent: 'flex-end' }}>
                        <button type="button" style={btn('transparent', 'var(--text-2)', 'var(--border)')} onClick={() => setEditSubjectModal({ open: false, subject: null, sem: null, name: '', shortCode: '', code: '', credits: 0, department: '', hasLab: false, labOnly: false })}>Cancel</button>
